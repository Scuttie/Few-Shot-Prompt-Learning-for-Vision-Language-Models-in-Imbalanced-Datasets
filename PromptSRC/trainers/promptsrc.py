...
class PromptSRC(TrainerX):
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.PROMPTSRC.PREC in ["fp32", "amp"]:
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            L_SCL_logits = L_SCL_logits * self.cfg.TRAINER.PROMPTSRC.LOGITS_LOSS_WEIGHT
        ...
        # =======================================================
        # GPA 로직 (USE_GPA == True 일 때만 수행)
        # =======================================================
        if self.cfg.TRAINER.PROMPTSRC.USE_GPA:
            # 에폭 끝났는지 판단
            ...
            # 만약 GPA를 끈 상태라면, 평소처럼 epoch 끝나면 LR만 업데이트
            if (self.batch_idx + 1) == self.num_batches:
                self.update_lr()
