...
class CustomCLIP(nn.Module):
    """IVLP에서의 CLIP + PromptLearner 결합 모델 (기존 IVLP 로직 유지)."""
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
...
class IVLP(TrainerX):
    """IVLP Trainer (few-shot 세팅이 config나 스크립트로 주어져도 그대로 학습 가능)."""
    ...
    def build_model(self):
        ...
        for name, param in self.model.named_parameters():
            if name_to_update not in name:
                # 혹시 "VPT" 등 다른 부분을 학습하고 싶다면 True로 바꿔도 됨
                if "VPT" in name:
