...
class UCF101(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        if num_shots > 0:
            ...
            if len(per_class_shots) > 0:
                ...
                print("[UCF101] num_shots<0 but no per_class_shots => skip")
    ...
    def read_data(self, cname2lab, text_file):
        ...
        with open(text_file, "r") as f:
            lines = f.readlines()
            for line in lines:
                # 예: "ApplyEyeMakeup/v_ApplyEyeMakeup_g08_c01.avi 91"
                # 여기서 filename만 추출
                line = line.strip().split(" ")[0]
                action, filename = line.split("/")
                label = cname2lab[action]

                # action="ApplyEyeMakeup" -> 정규식 분리 등
                ...
                items.append(Datum(impath=impath, label=label, classname=renamed_action))
