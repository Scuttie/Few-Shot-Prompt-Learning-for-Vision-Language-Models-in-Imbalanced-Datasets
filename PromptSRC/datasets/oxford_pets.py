...
class OxfordPets(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        # -----------------------------------------------------
        # [수정 포인트 A]
        # cfg.DATASET.NUM_SHOTS = int (기존처럼)
        # cfg.DATASET.PER_CLASS_SHOTS = list (디폴트 [])
        # -----------------------------------------------------
        ...
        # -------------------------------------------------------------
        # [수정 포인트 B]
        # "클래스별 샷(per_class_shots)" vs "단일 정수 샷(num_shots)" 구분
        # -------------------------------------------------------------
        if len(per_class_shots) > 0:
            # ---------------------------------------------------------
            # (1) 클래스별 shot 리스트가 주어진 경우
            # ---------------------------------------------------------
            ...
            if os.path.exists(preprocessed):
                ...
                # 예: val 데이터에는 너무 많지 않도록 min(기존값,4) 적용
